...
2. 在首页输入学习主题（如「Go 并发」），或打开「课程目录」挑一门开练
3. 选节点开始 AI 教练对话；完成练习后节点点亮
4. 可选：侧栏打开行动助手，梳理今日学习后再开练
5. 额度用尽时按提示填写自己的 LLM Key 继续使用
...
| IM 机器人 | ❌ 未开放 | ✅ Telegram / 钉钉 / 飞书 / 企微 |
| 管理员 | 管理控制台 + `ADMIN_TOKEN` | — |
...
维护者访问管理控制台，使用 `ADMIN_TOKEN` 登录查看用户与 Token 消耗。
