...
| 学习画像 | 记住背景与目标，裁剪讲解；可结构化编辑 | `#/settings/profile` |
| [行动助手](./action-assistant.md) | 过载分流、钉北星、勾选今日/清障 | [详细说明](./action-assistant.md) |
| 学习捷径 | 侧栏「上一节」续课 +「今日推荐」 | 见下方 |
...
侧栏提供两类快捷入口，减少「打开应用后不知道点哪」：

| 区域 | 说明 |
|------|------|
| 上一节 | 最近有进度的教练会话，可直接续练未完成节点 |
| 今日推荐 | 优先取自[行动助手](./action-assistant.md)计划中的今日学习；否则按课程进度推荐（最多 2 条） |

行动助手（`#/assistant`）用多轮对话产出北星、清障、今日行动；四象限为展开详情。目标是**恢复学习节奏**，不是通用待办。完整说明见 **[行动助手](./action-assistant.md)**。
