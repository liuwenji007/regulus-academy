...
下面说明它为什么一次只教一个知识点、为什么必须练习，以及如何判断你是否学会。
...
| 层级 | 典型时长（量级参考，非计时保证） | 目标 |
|------|----------|------|
| **入门** | ~2 小时量级 | 建立概念骨架，能看懂代码、能简单使用 |
| **熟悉** | ~8 小时量级 | 掌握生产中最常用的技能，能独立干活 |
| **精通** | ~20 小时量级 | 深入原理与边界，能处理高难度问题 |
...
系统用**两层画像**裁剪建课与讲解：全局背景/目标（摘要合计约 ≤500 字）+ **按课掌握摘要**（每课约 ≤200 字）。设置页可结构化编辑。

完整说明（冷启动、节末合并、设置页字段）见 **[学习画像](./learning-profile.md)**。
...
- 画像怎么改：[学习画像](./learning-profile.md)
- 差异化短述：[为什么是 Regulus](./why-regulus.md)
