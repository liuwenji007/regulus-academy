...
- [ ] **课程知识点考核可视化** — 掌握点进度可查看；节点维度展示考过/未考、掌握层级与薄弱点分布
- [ ] **学习进度按课程隔离** — 复考或传给模型时，仅注入当前课程进度 + 必要关联课程（前置依赖 / 跨域引用），避免全量画像噪声
- [ ] **建课资料知识库** — 上传与管理建课 / 考试素材（文档、题库、参考资料）；与导入建课、节点出题联动
- [ ] **行动助手看板化** — 四象限与今日行动从对话侧栏升级为可维护看板；聚焦目标、勾选完成、跨会话追踪
- [ ] **课程节点可视化管理** — 图谱 / 树形编辑器；重新生成课程时可附带用户反馈；支持手动拖拽、增删、调序（保留 `node_key` 与进度）
- [ ] **Skill 库管理** — 内置 / 自建 / 导入 Skill 的统一浏览、版本与启用；与建课裁剪、Coach 导出打通
...
- [ ] Embedding + RAG（用户上传自己的代码 / 笔记 / 文档作为补充材料；与「建课资料知识库」可合并演进）
- [ ] **可信信息源 + 联网搜索** — 白名单来源（官方文档、arXiv、技术博客等）；建课 / 讲解 / 行动助手按需检索，控制脏数据与成本
...
> 已验证产品方向，根据用户反馈逐步推进。2026-07-08 补充的八项方向已并入 Phase 4 中优 / 待验证；本节保留主题归类与依赖说明。

### 学习进度与考核

| 方向 | 说明 | 状态 |
|------|------|------|
| 考核可视化 | 课程内掌握点进度、考验证记录、薄弱点分布可查看 | Phase 4 中优 |
| 进度按课程隔离 | 复考 / Prompt 注入仅传当前课 + 必要关联课，降低上下文噪声 | Phase 4 中优 |

### 内容与知识库

| 方向 | 说明 | 状态 |
|------|------|------|
| 建课资料库 | 上传、分类、管理建课与考试素材；与导入建课、出题联动 | Phase 4 中优 |
| 可选 RAG | 用户自有代码 / 笔记作补充材料；Embedding 可选项 | 待验证 |
| 可信信息源 | 白名单联网搜索；建课 / 讲解按需检索 | 待验证 |

### 行动助手与课程编辑

| 方向 | 说明 | 状态 |
|------|------|------|
| 看板化 | 四象限 + 今日行动独立看板，勾选完成、跨会话维护 | Phase 4 中优 |
...
| Skill 库管理 | 内置 / 自建 / 导入 Skill 统一管理与启用 | Phase 4 中优 |
| Skill 市场集成 | 发布到 hermes 等 Agent 框架；社区可复用 skill 包 | Phase 4 高优（进行中） |

### 课程深度提升（延续）

- [ ] **建课阶段联网搜索** — 归入「可信信息源」；优先建课场景，不全程开放以控制脏数据
- [ ] **可选的 RAG 补充** — 与建课资料库合并演进；不违反「一个 Key 就能用」原则
- [ ] **Skill 系统集成** — 与 Skill 库管理、市场发布一并推进
