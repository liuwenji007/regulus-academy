...
```
Phase 0 · 项目立项        Phase 1 · 后端+Skill       Phase 1.5 · Channel       Phase 2 · 首个闭环       Phase 3 · 开源
  今天                      本周                      第 2 周                   下周末                     第 3 周
  ┌──────────────┐        ┌──────────────┐        ┌──────────────┐        ┌──────────────┐        ┌──────────────┐
  │ ✓ 竞品分析    │        │ ✓ Go 后端启动 │        │ □ 企微机器人   │        │ ✓ Go 并发知识树│        │ □ GitHub 公开 │
  │ ✓ 设计理念    │        │ ✓ Skill 骨架  │        │ □ 飞书/钉钉    │        │ ✓ 教学 Agent  │        │ □ 提交到精选集│
  │ ✓ 记忆管理    │        │ ✓ 知识域打包  │        │ □ 跨端共享记忆│        │ ✓ 记忆持久化  │        │ □ 社交媒体分享│
  │ ✓ 贡献手册    │        │ ✓ 单元测试    │        │              │        │ ✓ 错题强化    │        │ □ 收集早期反馈│
  │ ✓ 许可证      │        │              │        │              │        │ ✓ Channel闭环 │        │              │
  └──────────────┘        └──────────────┘        └──────────────┘        └──────────────┘        └──────────────┘
```
...
> 目标：Go 后端启动 + regulus-coach Skill 骨架。不做前端，教学对话通过 HTTP API 和 Skill 调用。

### 1.1 后端

- [x] Go 项目初始化（`go mod init`）
- [x] 项目目录结构搭建
- [x] SQLite 数据库初始化（用户表、进度表、错题表）
- [x] HTTP 路由框架（标准库 `net/http`）
- [x] DeepSeek API 调用封装
- [x] API 接口定义
...
> 目标：完成「Go 并发」完整教学闭环，通过 Skill 和 Channel 两种入口可用。
