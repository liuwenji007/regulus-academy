...
//go:embed migrations/012_domain_build_jobs.sql
var schemaSQL012 string
...
//go:embed migrations/027_domain_source_materials.sql
var schemaSQL027 string
...
func (s *Store) migrate() error {
	...
	if schemaSQL012 != "" {
		if _, err := s.db.Exec(schemaSQL012); err != nil {
			return fmt.Errorf("执行迁移 012 失败: %w", err)
		}
	}
	if schemaSQL013 != "" {
	...
	if schemaSQL027 != "" {
		if _, err := s.db.Exec(schemaSQL027); err != nil {
			if !strings.Contains(err.Error(), "already exists") {
				return fmt.Errorf("执行迁移 027 失败: %w", err)
			}
		}
	}
	return nil
