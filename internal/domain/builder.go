...
func buildTreePrompt(intent IntentResult, userInput, profile string, preserveKeys []string, refOutline string) string {
	...
	minModules, maxModules := moduleCountBounds(scope)
	...
	b.WriteString("）\n")
	fmt.Fprintf(&b, "主题模块 modules 数量建议：%d～%d 个（按本主题广度参考；硬性允许 %d～%d 个）\n",
		minModules, maxModules, moduleCountHardMin, moduleCountHardMax)
	if intent.Reason != "" {
		b.WriteString("学习意图：")
...
`)
	fmt.Fprintf(&b, "- modules 数组长度必须在 %d～%d 之间（与领域广度无关）\n", moduleCountHardMin, moduleCountHardMax)
	b.WriteString(`- 必须包含 entry、intermediate、advanced 三层
